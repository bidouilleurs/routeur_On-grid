...
float RAMesureClass::mesurePinceAC(int pinPinceAC, float coeff, bool avecsigne){
  ...
  int Tmax=20000; //us periode 50hz = 20ms = 20 000 us
   ...
   //    for(int i=1;i<ECH;i++) valuesA[i-1]=(valuesA[i]+valuesA[i-1])/2; 
...
/*     Serial.println(1000);
 ...
 //   delay(1000);
     ...
     for(int i=0;i<ECH;i++) {Serial.print(1);Serial.print(','); Serial.print(1);Serial.print(',');Serial.println(1);}
  */
      if (p>=0) signe+=1; else  signe-=1;    // teste 3 fois le signe
       ret+=abs(abs(I1valeff)-0.1);
       puissanceAC+=abs(p);
      }
      
     if (signe>=0) signe=1; else  signe=-1;    
     if (!avecsigne) signe=1;
     puissanceAC=signe*puissanceAC/nbfois;
     intensiteAC=signe*ret/nbfois;
       return(signe*ret/nbfois);
}
